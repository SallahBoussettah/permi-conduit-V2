        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-semibold text-gray-800">{{ __('Créer un nouveau cours') }}</h2>
                ...
                <form action="{{ route('inspector.courses.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description') }}</textarea>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="permit_category_id" id="permit_category_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    <option value="">{{ __('Aucune - Disponible pour tous') }}</option>
                                    @foreach($permitCategories as $id => $name)
                                    <option value="{{ $id }}" {{ old('permit_category_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                                ...
                                <div class="mt-1 flex items-center">
                                    <div class="w-full">
                                        <div id="thumbnail-preview" class="hidden mb-4">
                                            <img src="#" alt="Thumbnail Preview" class="w-40 h-auto object-cover rounded-md border border-gray-300">
                                        </div>
                                        <div class="flex items-center justify-center w-full">
                                            <label for="thumbnail" class="flex flex-col items-center justify-center w-full h-40 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                                                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                                    ...
                                                    <p class="mb-2 text-sm text-gray-500"><span class="font-semibold">{{ __('Cliquer pour télécharger') }}</span> {{ __('ou glisser-déposer') }}</p>

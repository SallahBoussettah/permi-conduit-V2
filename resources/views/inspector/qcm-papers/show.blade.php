        ...
        <div class="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow overflow-hidden sm:rounded-lg mb-6">
                <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg leading-6 font-medium text-gray-900">{{ __('Informations sur le papier QCM') }}
                        </h3>
                        <p class="mt-1 max-w-2xl text-sm text-gray-500">{{ __('Détails sur le papier QCM.') }}</p>
                    </div>
                    <div>
                        <span
                            class="inline-flex items-center px-3 py-0.5 rounded-full text-sm font-medium {{ $qcmPaper->status ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                    ...
                    <dl>
                        ...
                        <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            ...
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                {{ $qcmPaper->description ?? __('Aucune description fournie') }}</dd>
                        </div>
                        <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            <dt class="text-sm font-medium text-gray-500">{{ __('Catégorie de permis') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                {{ $qcmPaper->permitCategory->name }}</dd>
                        </div>
                        <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            ...
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                {{ $qcmPaper->created_at->format('d/m/Y H:i') }}</dd>
                        </div>
                        <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            <dt class="text-sm font-medium text-gray-500">{{ __('Dernière mise à jour') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                {{ $qcmPaper->updated_at->format('d/m/Y H:i') }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
            ...
            <div class="bg-white shadow overflow-hidden sm:rounded-lg">
                <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
                    ...
                    <div class="flex space-x-3">
                        <a href="{{ route('inspector.qcm-papers.questions.create', ['qcmPaper' => $qcmPaper->id]) }}"
                            class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            {{ __('Ajouter une question') }}
                        </a>
                        <a href="{{ route('inspector.qcm-papers.questions.import', ['qcmPaper' => $qcmPaper->id]) }}"
                                        ...
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div class="flex justify-end space-x-2">
                                                    <a href="{{ route('inspector.qcm-papers.questions.edit', ['qcmPaper' => $qcmPaper->id, 'question' => $question->id]) }}"
                                                        class="text-indigo-600 hover:text-indigo-900">{{ __('Modifier') }}</a>
                                                    <form
                                                        action="{{ route('inspector.qcm-papers.questions.destroy', ['qcmPaper' => $qcmPaper->id, 'question' => $question->id]) }}"
                                                        method="POST" class="inline-block"
                                                        onsubmit="return confirm('{{ __('Voulez-vous vraiment supprimer cette question ?') }}');">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                                                            class="text-red-600 hover:text-red-900">{{ __('Supprimer') }}</button>
                                                    </form>
                                                </div>
                                            </td>
                                        </tr>
